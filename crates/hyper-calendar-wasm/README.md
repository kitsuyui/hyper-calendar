...
16 functions. Types are the WebAssembly ones: `i64` crosses into JavaScript as a `BigInt`, everything else as a `number`, and a pointer is a byte offset into `memory`.
...
The surface is the civil calendar and the TAI–UTC bridge. The other calendars,
holidays, solar terms and locales are compiled in by the `full` feature and
reachable by adding exports of the same shape. The C ABI in
