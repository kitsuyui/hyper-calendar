# Coverage audit — 2026-09-21

> **Superseded snapshot. Do not read this as current.** It is kept for its
> research — the missing-calendar survey in §3 and the abstraction limits in
> §4 — and for the record of what was known when. Its §2 reconciliation
> between the documents and the registry is now done continuously by
> [`../supported.md`](../supported.md), which is generated from the code.
>
> Several of its findings have since inverted. Both of its top-two priorities
> were implemented: `DateFields` has a `leap_day` flag, and the day-boundary
> field exists — as a trait method, the shape this audit recommended against,
> with the reasoning written at `crates/hc-calendar/src/daystart.rs`. It calls
> `hc-almanac` a placeholder; it is a ten-module crate. It gives the crate
> count as twenty. A later audit found ten of its rows describing a version of
> `calendars.md` that no longer exists.
>
> The lesson is in its own §2: a hand-run reconciliation is correct for about
> a day. That is why the file has a date in its name now.
