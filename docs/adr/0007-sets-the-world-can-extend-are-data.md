...
its source — and `hc_core::catalogue_tests!` adds the same tests to a table
that has to stay hand-assembled, so that being off the macro is not being
off the guarantees. The macro exists so that declaring an
