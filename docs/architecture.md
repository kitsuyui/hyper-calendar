...
(the default), `timestamps`, `calendars`, `holiday`, `seasons`, `deep-time`, `tz`, `sky`,
`orbital`, `planetary`, `relativity` and `full` — so a page or a host program builds only the layer it loads,
