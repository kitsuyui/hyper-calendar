...
law gives a day in its place. Korea had a short-lived 익일휴무제, a
next-day rest, from 1989, abolished by the amendment of 5 November 1990
in the name of more working days; from 1991 to 2013 nothing replaced it
[kr-holiday-regulation-history]. The substitute holiday then came in
three steps, each an amendment to Article 3 of the decree:
...
from 2026. Each rule that the 대체공휴일 reaches carries
`substituted_from` with the first year of its step — 2014, 2021, 2023 or
2026 — and the two that it never reaches, 신정 and 현충일, are
`fixed_public`, which opts them out. The Seollal and Chuseok days carry
`substitute_on(SUNDAY_ONLY)`, their own trigger set, because a Saturday
never counted for them. Election days and designated days are `kr_one_off`
rules — `fixed_public`, bounded to one year — so they are never moved and
never trigger a substitute on their own.

**The policy.** `KR_SUBSTITUTION` is one `SubstitutionPolicy` valid from
2014: trigger Saturday and Sunday, direction `Forward`, `skip_occupied`
true, `on_collision` true. The engine (`evaluate` in `engine.rs`) takes the
...
- **The named days**, as rules bounded to the years each was a holiday,
  ...
  The rules are not bounded below except where the decree's history gives
  a start — Seollal from 1985 and 1989, Constitution and Hangul Day from
  1949, and Children's Day and Buddha's Birthday from 1975, a year the
  table carries and no source read here confirms — so for years before a
  rule's start the table is silent rather than wrong, and for 1949 onward
  it answers with the days it has.
- **The 대체공휴일** in its three steps and the collision rule, from 2014.
...
- **Designated days** from 2009, the first year the Korea Exchange's
  ...
  entered.
- **Not carried, and why:**
  - The holidays the decree dropped before the table's history begins to
    matter: 식목일 (5 April, to 2005), 국군의 날 (1 October, to 1990, and
    only the 2024 one-off since) and 2 January (to 1998), and the
    1989–1990 익일휴무제. The amendment list names them; their texts were
    not read, and the table carries only what was checked against a list.
  - Designated days before 2009, for the same reason: nothing to check
    them against.
...
working day, three election and designated days, and the substitutes of
Monday 12 February 2024 (a Sunday day of Seollal), Monday 3 March 2025 (a
Saturday 삼일절, under the 2021 step), 6 May 2025 and 6 October 2017 (the
two collisions above), 19 July and 3 May 2027 (the 2026 additions on a
weekend), and no substitute for Sunday 3 October 2010.
...
- The 2021 step was enacted twice: the Act of 7 July 2021, in force
  ...
  at once. The table dates the step 2021; the module comment's "July 2021"
  is the Act.
...
- 2 January was a holiday to 1998 and 5 April to 2005; the table does not
  carry them, so it under-reports the years before 2006 by those days.
...
| [wikipedia-ko-public-holidays] | The reasons for the designated days and the vacancy elections; the Hangul Day and Constitution Day gaps | Yes, 2026-09-25; secondary, for reasons only — the dates themselves are checked against the exchange |
